    ...
    methods: {
        ...
        setTransform(transform) {
            ...
            if (!this.data.disable_rotate) {
                this.setData({ angle: transform.angle ? this.data.angle + transform.angle : this.data.angle });
            }
            ...
            if (transform.cutX) {
                this.setData({ cut_left: cutX + transform.cutX });
                this.data.watch.cut_left(null, this);
            }
            if (transform.cutY) {
                this.setData({ cut_top: cutY + transform.cutY });
        ...
        _setCutCenter() {
            let cut_top = (this.data.info.windowHeight - this.data.height) * 0.5;
            let cut_left = (this.data.info.windowWidth - this.data.width) * 0.5;
            this.setData({ cut_top: cut_top, cut_left: cut_left });
        },
        setWidth(width) {
        ...
        setLimitMove(value) {
            this.setData({ _cut_animation: true, limit_move: !!value });
        },
        imgReset() {
            this.setData({
                scale: 1,
        ...
        setAngle(angle) {
            if (!angle) return;
            this.setData({ _cut_animation: true, angle: angle });
            this._imgMarginDetectionScale();
            !this.data._canvas_overflow && this._draw();
        },
        _initCanvas() {
        ...
        _cutDetectionPosition() {
            let _cutDetectionPositionTop = () => {
                if (this.data.cut_top < 0) { this.setData({ cut_top: 0 }); }
            ...
            _cutDetectionPositionLeft = () => {
                if (this.data.cut_left < 0) { this.setData({ cut_left: 0 }); }
            ...
            if (this.data.cut_top == null && this.data.cut_left == null) {
                ...
                _cutDetectionPositionTop();
                this.setData({ cut_left: (this.data.info.windowWidth - this.data.width) / 2 });
            } else if (this.data.cut_top == null && this.data.cut_left != null) {
                _cutDetectionPositionLeft();
                this.setData({ cut_top: (this.data.info.windowHeight - this.data.height) / 2 });
            }
        },
        _canvasDetectionPosition() {
            if (this.data.canvas_top == null && this.data.canvas_left == null) {
                this.data._canvas_overflow = false;
                this.setData({ canvas_top: -5000, canvas_left: -5000 });
        ...
        _imgMarginDetectionPosition(scale) {
            ...
            this.setData({ _img_left: left, _img_top: top, scale: scale });
        },
        ...
        _imgComputeSize() {
            ...
            this.setData({ img_width: img_width, img_height: img_height });
        },
        _computeCutSize() {
        ...
        _move(event) {
            ...
            if (event.touches.length == 1) {
                ...
                this.setData({ _img_left: this.data._img_left, _img_top: this.data._img_top });
            } else {
                let width = (Math.abs(event.touches[0].clientX - event.touches[1].clientX)),
                ...
                this.data._hypotenuse_length = Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2));
                this.setData({ angle: this.data.angle + current_deg, scale: this.data.scale });
            }
            !this.data._canvas_overflow && this._draw();
        },
        _end(event) {
        ...
        _click(event) {
            ...
            this._draw(() => {
                ...
                if ((x >= this.data.cut_left && x <= (this.data.cut_left + this.data.width)) &&
        ...
        _cutTouchMove(e) {
            if (this.data._flag_cut_touch && this.data.MOVE_THROTTLE_FLAG) {
                ...
                case 1:
                    ...
                    if (this.data.disable_ratio) { height = width / (this.data.width / this.data.height); }
                ...
                if (!this.data.disable_width && !this.data.disable_height) {
                    this.setData({ width: width, cut_left: cut_left, height: height, cut_top: cut_top });
                } else if (!this.data.disable_width) {
                    this.setData({ width: width, cut_left: cut_left });
                } else if (!this.data.disable_height) {
                    this.setData({ height: height, cut_top: cut_top });
                }
                this._imgMarginDetectionScale();
            }
        },
        _cutTouchStart(e) {
            ...
            if (currentX > cutbox_left4 && currentX < cutbox_right4 && currentY > cutbox_top4 && currentY < cutbox_bottom4) {
                ...
                this.data._flag_img_endtouch = true;
                this.data.CUT_START = { width: this.data.width, height: this.data.height, x: currentX, y: currentY, corner: 4 };
            } else if (currentX > cutbox_left3 && currentX < cutbox_right3 && currentY > cutbox_top3 && currentY < cutbox_bottom3) {
                this._moveDuring();
                this.data._flag_cut_touch = true;
                this.data._flag_img_endtouch = true;
                this.data.CUT_START = { width: this.data.width, height: this.data.height, x: currentX, y: currentY, cut_top: this.data.cut_top, cut_left: this.data.cut_left, corner: 3 };
            } else if (currentX > cutbox_left2 && currentX < cutbox_right2 && currentY > cutbox_top2 && currentY < cutbox_bottom2) {
                this._moveDuring();
                this.data._flag_cut_touch = true;
                this.data._flag_img_endtouch = true;
                this.data.CUT_START = { width: this.data.width, height: this.data.height, cut_top: this.data.cut_top, cut_left: this.data.cut_left, x: currentX, y: currentY, corner: 2 };
            } else if (currentX > cutbox_left1 && currentX < cutbox_right1 && currentY > cutbox_top1 && currentY < cutbox_bottom1) {
                this._moveDuring();
                this.data._flag_cut_touch = true;
                this.data._flag_img_endtouch = true;
                this.data.CUT_START = { width: this.data.width, height: this.data.height, cut_top: this.data.cut_top, cut_left: this.data.cut_left, x: currentX, y: currentY, corner: 1 };
            }
        },
        _cutTouchEnd(e) {
            this._moveStop();
